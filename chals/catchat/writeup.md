...
We started by analyzing both the client and the server code. Then we solved the challenge by backtracking from the point, where we have an admin cookie and needs to get it to the user. Our overall approach can be described as:

1. Identify vulnerabilities in the code
2. Find a way to leak the admin cookie to the html
3. Find a way to read the cookie from html and save it in a chat-message
4. Exploit

We will be going through each of them below.

## Identify vulnerabilities in the code
### Client-side code
As we wanted to leak a cookie, we started by looking for XSS in the client-side code:
```javascript
// Set name
...
We now have an injection in the form of CSS! Due to the content security policy (CSP), we are not allowed to load css from other domains:
```
...
It is worth noting that this only occurs when the admin bans a person. In order to get the admin to ban a person, the admin must be present in our room. We can achieve this by writing ```/report``` in a message. And our target person, then has to write a sentence containing the word 'dog'.

### Server-side code
The server is running the following script
```javascript
...
## Find a way to leak the admin cookie to the html
As we do not have XSS and cannot load a cookie using CSS, we need to rely on some code for returning the contents of the cookie. Fortunately, calling ```/secret someValue``` will write the contents of the flag-cookie to our HTML. Looking at how it is coded on the server, we identify another vulnerability, as ```someValue``` is not escaped. By defining a domain in addition to the value, we set a new cookie instead of overwriting the flag-cookie. This will effectively print the real flag to the HTML:

```
/secret deadbeef; Domain=foo.web.ctfcompetition.com
```

However, we have to make the admin call this command. And using the CSS to make a call, will not make the admin client process the response. Thus, we need to take advantage of the multiple commands chain vulnerability identified above. By setting our name to the above payload, the admin will send the following payload to the server, when banning us:
```
/ban /secret deadbeef; Domain=foo.web.ctfcompetition.com
```

This will ban the user but will also make the admin leak his cookie to the HTML. The resulting HTML will look similar to this:
...
## Find a way to read the cookie from html and save it in a chat-message
Now having CSS-injection and having a cookie in an html-element on the website using CSS-selectors. That is a CSS-selector of ```span[data-secret^=A]``` will match any span with a ```data-secret``` attribute beginning with the letter A. This can be used to load an url if found:
```css
span[data-secret^=A] {
...
This step could be done by manually posting the command ```/report``` in the chat-window, but it is of course much more fun to automate. As this is a simple captcha, we can simple point our browser to the chatroom and execute the following javascript in the Developer Console:
```javascript
setInterval(() => {
    grecaptcha
...
3. Leak the admin flag-cookie to the HTML, by making the admin ban our user
4. Join with a new user, which uses CSS injection to leak one character of our flag at a time
5. Ban the new css-leaking user 
6. Repeat 4-5 until we have the whole flag
