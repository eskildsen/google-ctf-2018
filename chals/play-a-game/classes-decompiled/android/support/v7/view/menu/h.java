    ...
    void a(final List<j> list, final int n, final KeyEvent keyEvent) {
        ...
        if (keyEvent.getKeyData(keyCharacterMap$KeyData) || n == 67) {
            for (int size = this.j.size(), i = 0; i < size; ++i) {
                ...
                int n2;
                if (b) {
                    n2 = j.getAlphabeticModifiers();
                }
                else {
                    n2 = j.getNumericModifiers();
                }
                int n3;
                ...
                if (n3 != 0 && c != '\0' && (c == keyCharacterMap$KeyData.meta[0] || c == keyCharacterMap$KeyData.meta[2] || (b && c == '\b' && n == 67)) && j.isEnabled()) {
                    list.add(j);
                }
            }
        }
